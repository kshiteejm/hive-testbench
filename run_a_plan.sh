...
find query12.sql | while read line; do
    ...
    (hive --hiveconf hive.execution.engine=tez --hiveconf hive.stats.fetch.column.stats=true \
        --hiveconf hive.session.id="$query.0" \
        ...
        --hiveconf hive.server2.tez.initialize.default.sessions=true \
        --hiveconf hive.server2.tez.default.queues=default \
        --hiveconf hive.server2.tez.sessions.per.default.queue=4 \
        -f $line \
